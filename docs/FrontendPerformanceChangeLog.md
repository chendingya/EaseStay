...
| 2026-02-24 | Merchants 首屏关键路径 | `admin/src/pages/Merchants.jsx`、`admin/src/components/merchants/MerchantsTable.jsx`、`admin/src/components/merchants/ResetPasswordModal.jsx` | 商户管理页将 `Table` 与重置密码弹窗拆到懒加载子组件；首屏先渲染标题与筛选区，表格空闲时挂载 | 降低 `/merchants` 首屏 JS 执行与表格相关依赖注入，改善 FCP/LCP 与 Unused JS | `npm run build` 通过；`npm test` 通过 |
| 2026-02-24 | GlassUI 依赖拆分 | `admin/src/components/GlassUI.jsx`、`admin/src/components/GlassButton.jsx`、`admin/src/components/GlassCard.jsx`、`admin/src/components/GanttTimeline.jsx` | 将 `GlassButton/GlassCard` 与 `GanttTimeline(dayjs)` 拆分为独立模块，避免仅使用按钮时加载甘特图与日期库逻辑 | 减少业务页因 `GlassButton` 触发的额外 Unused JS，优化网络依赖树 | `npm run build` 通过；`npm test` 通过 |
| 2026-02-24 | Merchants 启动关键路径收敛 | `admin/src/main.jsx`、`admin/src/App.jsx`、`admin/src/pages/Merchants.jsx` | 启动阶段不再阻塞等待路由命名空间，改为先渲染框架后异步补齐路由文案；`services(axios)` 改为按需动态加载；`Merchants` 表格改为首轮数据返回后再空闲挂载 | 缩短 `/merchants` 首屏可渲染等待与主包解析压力，降低 FCP/LCP 与 Unused JS | `npm run build` 通过；`npm test` 通过 |
| 2026-02-24 | Merchants 表格实现回退 | `admin/src/components/merchants/MerchantsTable.jsx` | 按业务一致性要求将商户列表表格回退为 `antd Table` 方案（保留分页/快捷跳页与原交互） | 保持页面交互与全站表格体验一致，降低维护心智成本 | `npm run build` 通过；`npm test` 通过 |
